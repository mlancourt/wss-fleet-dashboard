...
> **Schema 6 (LIVE, additive):** `service_queue[]`, `leads[]` and `agreements[]` each gain `docs[]` — `{id, name, kind, bytes, added}`, always present, may be `[]`. `id` is 16 lowercase hex (the content hash); `kind` ∈ `QUOTE · WORKORDER · PARTS-LIST · PM-REPORT · SERVICE-TICKET · PO · PHOTO · WRITEUP · OTHER` (WRITEUP = the vault's post-invoice service write-up, added 2026-09-15; vault-minted only, renders via the 📄 fallback — no site change needed). Read at `GET /api/doc/<id>` (token, `?t=` or Bearer). **Docs are never stripped and never role-gated** — the §6 money gate does not touch them. A schema-5 snapshot (no `docs` key) must render unchanged. Upload: the documents block above.
